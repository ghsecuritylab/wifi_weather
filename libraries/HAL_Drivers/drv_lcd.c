...
/**
 * display a color point on the lcd.
 *
 * @param   x   x position
 * @param   y   y position
 * @param   color   color
 ...
 * @return  void
 */
void lcd_draw_colorpoint(rt_uint16_t x, rt_uint16_t y,rt_uint16_t color)
{
    lcd_address_set(x, y, x, y);
 ...
 * @return  void
 */
void lcd_fill_buf(rt_uint16_t x_start, rt_uint16_t y_start, rt_uint16_t x_end, rt_uint16_t y_end, rt_uint16_t* clr_buf)
{
    rt_uint16_t i = 0, j = 0;
    rt_uint32_t size = 0, size_remain = 0;
    rt_uint8_t *fill_buf = RT_NULL;
    ...
    if (fill_buf)
    {
        ...
        while (1)
        {
            for (i = 0; i < size / 2; i++)
            {
                fill_buf[2 * i] = clr_buf[i] >> 8;
                fill_buf[2 * i + 1] = clr_buf[i];
            }
            rt_pin_write(LCD_DC_PIN, PIN_HIGH);
            rt_spi_send(spi_dev_lcd, fill_buf, size);
 ...
 * @return 	void
 */
void lcd_show_font(rt_uint16_t x, rt_uint16_t y, rt_uint8_t *font, rt_uint8_t size)
{
    rt_uint16_t colortemp;
    rt_uint8_t sta;
    rt_uint8_t temp, t, t1;
    rt_uint8_t dzk[128];
    rt_uint8_t csize = (size / 8 + ((size % 8) ? 1 : 0)) * (size);	//得到字体一个字符对应点阵集所占的字节数

    if(size != 12 && size != 16 && size != 24 && size != 32)return;	//不支持的size

    Get_HzMat(font, dzk, size);							//得到相应的点阵大小

    if((size == 16) || (size == 24) || (size == 32))	//字体大小
    {
        sta = 8;
 ...
 * @return 	void
 */
void lcd_show_hz(rt_uint16_t x, rt_uint16_t y, rt_uint16_t width, rt_uint16_t height, rt_uint8_t*str, rt_uint8_t size)
{
    rt_uint16_t x0 = x;
    rt_uint16_t y0 = y;
    rt_uint8_t bHz = 0;  
       ...
       if(!bHz)
        {
            ...
            else            
            {
                ...
                else lcd_show_string(x, y, size,(char const*)gbk); 
